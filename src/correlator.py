...
@Revised 2017/12/13: Took `Correlator` class out of corr_functions.py in order
...
from . import log_handlers, cn_conf, corr_nb, corr_wb, snap
...
class Correlator(object):
    ...
    def __init__(self, connect = True, config_file = default_config, log_handler = None, log_level = logging.INFO):
        ...
        self.config = cn_conf.CorrConf(config_file)
    ...
    def connect(self):
        self.xfpgas=[katcp_wrapper.FpgaClient(server,self.config['katcp_port'],
                       timeout=10,logger=self.xloggers[s]) for s,server in enumerate(self.xsrvs)]
        self.ffpgas=[katcp_wrapper.FpgaClient(server,self.config['katcp_port'],
            ...
            if self.config['adc_type'] == 'katadc':
                for fn,fpga in enumerate(self.ffpgas):
                    for an in range(self.config['f_per_fpga']):
                        adc_details=katadc.eeprom_details_get(fpga,an)
    ...
    def ant_str_to_baseline(self, ant_tuple):
        '''e.g. ('3x', '6y') will return either the baseline (as generated by get_bl_order) or -1, if that pairing doesn't exist.
        '''
        try:
            return self.get_bl_order().index(ant_tuple)
        except:
            return -1

    def baseline_to_ant_str(self, baseline):
        try:
            return self.get_bl_order()[baseline]
        except:
            return ('n/a', 'n/a')
    ...
    def feng_ctrl_set_all(self, **kwargs):
        ...
        """
        if self.is_wideband():
            write_masked_register(self.ffpgas, corr_wb.register_fengine_control, **kwargs)
        elif self.is_narrowband():
            write_masked_register(self.ffpgas, corr_nb.register_fengine_control, **kwargs)
        else:
            raise RuntimeError('Unknown mode. Cannot write F-engine control.')

    def feng_ctrl_get_all(self):
        if self.is_wideband():
            return read_masked_register(self.ffpgas, corr_wb.register_fengine_control)
            #return corr_wb.feng_status_get(self, ant_str)
        elif self.is_narrowband():
            return read_masked_register(self.ffpgas, corr_nb.register_fengine_control)
            #return corr_nb.feng_status_get(self, ant_str)
        else:
    ...
    def feng_tvg_available_tvgs(self):
        reg = None
        if self.is_wideband():
            reg = corr_wb.register_fengine_control
        elif self.is_narrowband():
            reg = corr_nb.register_fengine_control
        else:
    ...
    def xeng_ctrl_set_all(self, **kwargs):
        write_masked_register(self.xfpgas, corr_wb.register_xengine_control, **kwargs)

    def xeng_ctrl_get_all(self):
        return read_masked_register(self.xfpgas, corr_wb.register_xengine_control)
    ...
    def fft_shift_set_all(self, fft_shift = -1):
        ...
        if self.is_wideband():
            ...
            corr_nb.fft_shift_coarse_set_all(self)
            corr_nb.fft_shift_fine_set_all(self)
    ...
    def fft_shift_get_all(self):
        if self.is_wideband():
            ...
            rv = corr_nb.fft_shift_get_all(self)
        else:
    ...
    def xeng_status_get_all(self):
        ...
        for loc_xeng_n in range(self.config['x_per_fpga']):
            for xfpga_num, srv in enumerate(self.xsrvs):
                ...
                rv[xeng_id] = read_masked_register([self.xfpgas[xfpga_num]], corr_wb.register_xengine_status, names = ['xstatus%i' % loc_xeng_n])[0]
    ...
    def rst_xstatus(self):
        ...
        if self.is_wideband():
            pulse_masked_register(self.xfpgas, corr_wb.register_xengine_control, ['cnt_rst', 'clr_status'])
        elif self.is_narrowband():
            pulse_masked_register(self.xfpgas, corr_nb.register_xengine_control, ['cnt_rst', 'clr_status'])
        else:
    ...
    def rst_fstatus(self):
        ...
        if self.is_wideband():
            pulse_masked_register(self.ffpgas, corr_wb.register_fengine_control, ['clr_status'])
        elif self.is_narrowband():
            pulse_masked_register(self.ffpgas, corr_nb.register_fengine_control, ['clr_status'])
        else:
    ...
    def rst_vaccs(self):
        ...
        if self.is_wideband():
            pulse_masked_register(self.xfpgas, corr_wb.register_xengine_control, ['vacc_rst'])
        elif self.is_narrowband():
            pulse_masked_register(self.xfpgas, corr_nb.register_xengine_control, ['vacc_rst'])
        else:
...
#        return round(float(frequency)/self.config['bandwidth']*self.config['n_chans'])%self.config['n_chans']
    ...
    def get_adc_snapshots(self,ant_strs,trig_level=-1,sync_to_pps=True):
        ...
        return snap.get_adc_snapshots(self,ant_strs,trig_level=trig_level,sync_to_pps=sync_to_pps)
    ...
    def get_quant_snapshot(self, ant_str, n_spectra = 1):
        ...
        return snap.get_quant_snapshot(self, ant_str, n_spectra = n_spectra)
    ...
    def spead_narrowband_issue(self):
        if self.is_narrowband():
            self.spead_ig.add_item(name="coarse_chans",id=0x1017,
                description="The number of coarse channels in a narrowband design.",
                shape=[],fmt=spead.mkfmt(('u',spead.ADDRSIZE)),
                init_val=self.config['coarse_chans'])
            self.spead_ig.add_item(name="current_coarse_chan",id=0x1018,
                description="The currently chosen coarse channel in a narrowband design.",
                shape=[],fmt=spead.mkfmt(('u',spead.ADDRSIZE)),
                init_val=self.config['current_coarse_chan'])
            self.spead_tx.send_heap(self.spead_ig.get_heap())
            self.syslogger.info("Issued narrowband-specific SPEAD metadata describing number of coarse channels (%i) and chosen coarse channel (%i)."%(self.config['coarse_chans'], self.config['current_coarse_chan']))
